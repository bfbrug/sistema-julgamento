...
export default function ResetJudgePasswordPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const router = useRouter()
  const { data: judge } = useUser(id)
  ...
  const onSubmit = async (data: FormData) => {
    ...
    setTimeout(() => router.push('/judges'), 1500)
  }
  ...
  return (
    <>
      <PageHeader
        ...
        description={judge ? `Definir nova senha para ${judge.name}` : 'Carregando...'}
      />
      <Card
        className="max-w-sm"
        body={
